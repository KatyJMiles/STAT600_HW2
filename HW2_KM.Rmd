...
namespace helper{
  colvec loglike_grad(colvec theta, mat data) {
      vec data_vec = data.col(0);
      int n = data_vec.size();
      colvec grad = colvec(3);
      for (int i = 0; i < n; i++) {
          double exp_part = std::exp(theta[0] + theta[1]*data(i,1) + theta[2]*data(i,2));
          grad[0] = grad[0] + std::log(data(i,0) - (exp_part / (1 + exp_part)));  
          grad[1] = grad[1] + std::log(data(i,0)*data(i,1) - (data(i,1)*exp_part / (1 + exp_part)));
          grad[2] = grad[2] + std::log(data(i,0)*data(i,2) - (data(i,2)*exp_part / (1 + exp_part)));
      }
      return grad;
  }
...
data = matrix(c(y,coffee, gender), ncol = 3)

newtonRaphson(data, c(.5,.7,1), .1)

loglike = function(theta) {
  sum(data[,1]*(theta[1] + theta[2]*data[,2] + theta[3]*data[,3]) - log(1 + exp(theta[1] + theta[2]*data[,2] + theta[3]*data[,3])))
}

optim(c(.5,.5,.5), loglike)
```
